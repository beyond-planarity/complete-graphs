...
public class NodeInserterGapPlanar extends NodeInserter {
	...
	private void createEdgeToFirstVertexHelper(Embedding emb, int faceId,
		...
		for (Edge e : incidentEdges) {
			...
			if (!embCrossingAdded.hasWholeEdgeGap(e.getId())) {
				// the current edge e has no gap -> make one
				Embedding newEmb = getCopy(embCrossingAdded);
				
				// make the gap; e gets the gap
				newEmb.setGapCrossingToEdge(newCrossing, e.getId());
				
				Embedding newEmbCopy = getCopy(newEmb);
			...
			if (!hasGap) {
				...
				for (int edgeId : newEmbCopy.getGapCrossingsToEdge().values()) {
					currentlyProhibitedEdges2.addAll(newEmbCopy.getWholeEdge(edgeId));
				}
		...
		for (Edge e : incidentEdges) {
			...
			if (!embCrossingAdded.hasWholeEdgeGap(e.getId())) {
				...
				if (embCrossingAdded.hasFaceVertex(faceOfTwin, finalTarget)) {
					...
					insertionPossible = true;
					if (PRINT) {
						System.out.println("Found new intermetiate embedding - ID: " + newFinalEmb.getId() + " (1 crossing 2nd edge)");
					}
				}
